...
with tab2:
    ...
    if "deep_dive" in st.session_state:
        ...
        implied = analysis.get("implied_prices", {})
        implied_valuations = implied.get("valuations", [])
        if implied_valuations:
            with st.expander("Mean Reversion Implied Prices", expanded=True):
                st.caption("Implied prices based on historical average ratios × current fundamentals.")

                # Global warnings
                for w in implied.get("warnings", []):
                    st.warning(w)

                # Summary metrics
                sum_cols = st.columns(3)
                with sum_cols[0]:
                    st.metric("Current Price", f"${implied.get('current_price', 0):.2f}")
                with sum_cols[1]:
                    median_p = implied.get("median_implied_price")
                    st.metric("Median Implied Price",
                              f"${median_p:.2f}" if median_p else "N/A")
                with sum_cols[2]:
                    upside = implied.get("upside_pct")
                    if upside is not None:
                        color = "normal" if abs(upside) < 15 else ("off" if upside < 0 else "normal")
                        st.metric("Median Upside/Downside", f"{upside:+.1f}%",
                                  delta=f"{upside:+.1f}%",
                                  delta_color="normal" if upside >= 0 else "inverse")
                    else:
                        st.metric("Median Upside/Downside", "N/A")

                # Individual valuations table
                val_rows = []
                for v in implied_valuations:
                    confidence_icon = {"high": "High", "normal": "Normal", "low": "Low"}.get(
                        v["confidence"], "Normal")
                    val_rows.append({
                        "Method": v["method"],
                        "Implied Price": f"${v['implied_price']:.2f}",
                        "Upside": f"{v['upside_pct']:+.1f}%",
                        "Confidence": confidence_icon,
                        "Note": v.get("warning") or "",
                    })
                val_df = pd.DataFrame(val_rows)
                st.dataframe(val_df, use_container_width=True, hide_index=True)

                # Visual: implied prices vs current
                fig_implied = go.Figure()
                for v in implied_valuations:
                    bar_color = {
                        "high": "rgba(0, 180, 0, 0.7)",
                        "normal": "rgba(100, 149, 237, 0.7)",
                        "low": "rgba(220, 50, 50, 0.5)",
                    }.get(v["confidence"], "rgba(100, 149, 237, 0.7)")
                    fig_implied.add_trace(go.Bar(
                        x=[v["method"]],
                        y=[v["implied_price"]],
                        marker_color=bar_color,
                        text=[f"${v['implied_price']:.2f}"],
                        textposition="outside",
                        showlegend=False,
                    ))
                fig_implied.add_hline(
                    y=implied.get("current_price", 0),
                    line_dash="dash", line_color="white",
                    annotation_text=f"Current: ${implied.get('current_price', 0):.2f}",
                )
                if implied.get("median_implied_price"):
                    fig_implied.add_hline(
                        y=implied["median_implied_price"],
                        line_dash="dot", line_color="yellow",
                        annotation_text=f"Median: ${implied['median_implied_price']:.2f}",
                    )
                fig_implied.update_layout(
